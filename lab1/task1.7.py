...
# sobel
grad_x = cv2.Sobel(
    m_gray, cv2.CV_16S, 1, 0, ksize=3, scale=1, delta=0, borderType=cv2.BORDER_DEFAULT
)
grad_y = cv2.Sobel(
    m_gray, cv2.CV_16S, 0, 1, ksize=3, scale=1, delta=0, borderType=cv2.BORDER_DEFAULT
)
...
# laplacian
laplacian = cv2.Laplacian(m_gray, cv2.CV_16S)
